...
public class Pesel implements Document {
    ...
    private Date extractBirthDate(String number) {
        char[] p = number.toCharArray();

        int year;
        int month = Integer.parseInt(String.valueOf(p[2]+p[3]));
        if (month - 20 > 0) {
            year = Integer.parseInt(String.valueOf("20"+p[0]+p[1]));
        } else {
            year = Integer.parseInt(String.valueOf("19"+p[0]+p[1]));
        }
        int dayOfMonth = Integer.parseInt(String.valueOf(p[4]+p[5]));

        return new Date(year, month, dayOfMonth);
    }

    private String extractSex(String number) {
//        char[] p = number.toCharArray();

        int sexNumber = number.charAt(9) - '0';

        return sexNumber % 2 == 0 ? "male" : "female";
    }
}